...
test("saving is an explicit local tool with the same scan input contract", () => {
  assert.equal(SCAN_SITE_AND_SAVE_TOOL.name, "scan_site_and_save");
  assert.deepEqual(SCAN_SITE_AND_SAVE_TOOL.inputSchema, SCAN_SITE_TOOL.inputSchema);
  assert.equal(SCAN_SITE_TOOL.annotations.readOnlyHint, true);
  assert.equal(SCAN_SITE_AND_SAVE_TOOL.annotations.readOnlyHint, false);
});
