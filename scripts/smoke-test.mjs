...
function runOfflineHandshake() {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      if (JSON.stringify(names) !== JSON.stringify(["scan_site", "scan_site_and_save"])) {
        return reject(new Error("MCP tools/list did not expose scan_site and scan_site_and_save."));
      }
      resolve(initialized);
    });
...
async function main() {
  ...
  if (scanIndex !== -1) {
    ...
    const profileArgument = process.argv[scanIndex + 2];
    const profile = profileArgument && !profileArgument.startsWith("--") ? profileArgument : "all";
    const toolName = process.argv.includes("--save") ? "scan_site_and_save" : "scan_site";
    if (!url) throw new Error("--scan requires a public URL and optional profile.");
    const result = await new AgentReadinessGateway().callTool(toolName, { url, profile });
  ...
  process.stdout.write(`PASS: offline MCP handshake exposes scan_site and scan_site_and_save through ${initialized.serverInfo.name} ${initialized.serverInfo.version}.\n`);
}
